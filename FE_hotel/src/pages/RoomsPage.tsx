...
const RoomsPage: React.FC = () => {
  ...
  const [unavailableRoomIds, setUnavailableRoomIds] = useState<Set<number>>(new Set());
  ...
  // Read roomType from URL query on mount
  useEffect(() => {
    const typeParam = searchParams.get('type');
    if (typeParam) {
      const upperType = typeParam.toUpperCase();
      const validTypes = Object.values(RoomType);
      if (validTypes.includes(upperType as RoomType)) {
        setSelectedType(upperType as RoomType);
      }
    }
  }, [searchParams]);
  ...
  // Mark rooms unavailable for selected range to allow overlay/hidden behavior
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const available = await roomService.searchRooms({
          checkInDate,
          checkOutDate,
          capacity: capacity ? parseInt(capacity) : undefined
        });
        const availableIds = new Set(available.map(r => r.id));
        const notAvailable = new Set<number>();
        rooms.forEach(r => { if (!availableIds.has(r.id)) notAvailable.add(r.id); });
        setUnavailableRoomIds(notAvailable);
      } catch (e) {
        console.warn('Không thể xác định tình trạng phòng:', e);
      }
    };
    run();
  }, [checkInDate, checkOutDate, capacity, rooms]);
  ...
  const filteredRooms = useMemo(() => {
    ...
    if (minPrice) {
      ...
      result = result.filter(r => r.pricePerNight >= min);
    }
    if (maxPrice) {
      const max = parseFloat(maxPrice);
      result = result.filter(r => r.pricePerNight <= max);
    }
    ...
    if (hideUnavailable && checkInDate && checkOutDate) {
      result = result.filter(r => !unavailableRoomIds.has(r.id));
    }
    
    return result;
  }, [rooms, selectedType, minPrice, maxPrice, capacity, hideUnavailable, checkInDate, checkOutDate, unavailableRoomIds]);
              ...
              <div className="row">
                ...
                <div className="col-lg-2 col-md-4">
                  <div className="form-group form-check mt-4">
                    ...
                    />
                    <label htmlFor="hideUnavailable" className="form-check-label">
                      Ẩn phòng đã được đặt
                    </label>
                  </div>
                </div>
              </div>
          ...
          {!loading && (
            <div className="rooms-list mt-5">
              <h4 className="mb-4">
                ...
                {checkInDate && checkOutDate && unavailableRoomIds.size > 0 && (
                  <span className="text-muted ms-2" style={{ fontSize: '0.9rem' }}>
                    ({unavailableRoomIds.size} phòng đã kín lịch)
                  </span>
                )}
              </h4>
              <div className="row">
                    ...
                    <div key={room.id} className="col-lg-4 col-md-6 mb-4">
                      <div className="room-card">
                        <div className="room-image">
                          ...
                          {checkInDate && checkOutDate && unavailableRoomIds.has(room.id) && !hideUnavailable && (
                            <div className="room-overlay">
                              <span className="overlay-text">ĐÃ ĐẶT</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="room-info">
                          ...
                          <div className="room-footer">
                            ...
                            <Link
                              ...
                              className={`btn-book ${checkInDate && checkOutDate && unavailableRoomIds.has(room.id) ? 'disabled' : ''}`}
                              onClick={(e) => {
                                if (checkInDate && checkOutDate && unavailableRoomIds.has(room.id)) {
                                  e.preventDefault();
                                  alert('Phòng này đã được đặt trong khoảng ngày bạn chọn. Vui lòng chọn phòng khác hoặc đổi ngày.');
                                }
                              }}
                            >
                              {checkInDate && checkOutDate && unavailableRoomIds.has(room.id) ? 'Không khả dụng' : 'Đặt Ngay'}
                            </Link>
                          </div>
                        </div>
                      </div>
